          ...
          <div className="hidden md:flex items-center gap-1">
            {links.map((l) => (
              <Link
                key={l.to}
                to={l.to}
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${isActive(l.to)
            ...
            <div className="flex items-center gap-2.5 ml-3">
              ...
              <Link
                to="/owner"
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${isActive('/owner')
                    ? 'bg-primary/10 text-primary'
                    : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'
                  }`}
              >
                    ...
                    <div className="py-1">
                      {profileMenuItems.map((item) => (
                        <Link
                          ...
                          className={`flex items-center gap-3 px-4 py-2.5 text-sm transition-colors ${isActive(item.to)
        ...
        <div className="md:hidden border-t border-gray-100 bg-white px-4 pb-4">
          {links.map((l) => (
            <Link
              ...
              className={`block px-4 py-2 rounded-lg text-sm font-medium mt-1 ${isActive(l.to) ? 'bg-primary/10 text-primary' : 'text-gray-600'
                }`}
            >
              {l.label}
            </Link>
            ...
            <div className="mt-3 border-t border-gray-100 pt-3">
              ...
              {profileMenuItems.map((item) => (
                <Link
                  ...
                  to={item.to}
                  onClick={() => setMobileOpen(false)}
                  className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium mt-1 ${isActive(item.to) ? 'bg-primary/10 text-primary' : 'text-gray-600'
                    }`}
                >
                  <span className="text-gray-400">{item.icon}</span>
                  {item.label}
